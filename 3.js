...
function calculateAdvanced(...cb) {
    ...
    let str_err = "It is not a function";
    for (let i = 0; i < cb.length; i++){
            try {
                ...
                let res = getLastResult(i);
                values.push(cb[i](res));
                if (typeof values[i] === 'undefined'){
                    ret.errors.push({
                        index: i,
                        name: "Error",
                        message: `callback at index ${i} did not return any value.`
                    });
                }
                else{
                    ret.value = values[i];
